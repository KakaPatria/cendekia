...
class PengerjaanController extends Controller
{
    public function create($id)
    {
        ...
        if (!$tryoutMateri->tryoutMaster->is_registered) {
            return redirect()->route('siswa.tryout.show', $tryoutMateri->tryout_id)
                ->withSuccess(('Maaf Anda tidak terdaftar dalam tryout ini'));
        }
        if ($tryoutMateri->tryoutMaster->tryout_status != 'Aktif') {
            return redirect()->route('siswa.tryout.show', $tryoutMateri->tryout_id)
                ->withSuccess(('Maaf Tryout sudah tidak aktif'));
        }
        if (!$tryoutMateri->in_periode) {
            return redirect()->route('siswa.tryout.show', $tryoutMateri->tryout_id)
                ->withSuccess(('Maaf Tryout sedang tidak dalam periode pengerjaan'));
        }
    ...
    public function jawab(Request $request, $id)
    {
        ...
        $pengerjaan->status = $soal->tryout_kunci_jawaban  ==  $request->jawaban ? 'Benar' : 'Salah';
        $pengerjaan->save();
    }
    ...
    public function leave($id)
    {
        ...
        return redirect()->intended(route('siswa.tryout.detail', $nilai->tryout_id))->withSuccess('Meninggalkan tryout');
    }
    public function selesai($id)
    {
        ...
        $benar = $pengerjaan->where('status', 'Benar')->count();

        $nilai->jumlah_salah = $salah;
        $nilai->jumlah_benar = $benar;
        $nilai->nilai = ($benar / $nilai->soal_total) * 100;
