...
<script>
    ...
    // === [IMPROVEMENT] LOGIKA VALIDASI BARU ===
    function validateStep(stepIndex) {
        const currentStepElement = steps[stepIndex];
        // Hanya validasi input yang terlihat di step saat ini
        const inputs = currentStepElement.querySelectorAll('input[required], select[required]');
        let isStepValid = true;

        inputs.forEach(input => {
            // Cek jika input visible (karena select2 menyembunyikan select asli)
    ...
    function showStep(stepIndex) {
        steps.forEach((step, index) => {
            ...
            if (step.classList.contains('active')) {
                // Inisialisasi dan tambahkan event listener ke step yang baru aktif
                validateStep(index);
                const inputs = step.querySelectorAll('input[required], select[required]');
                inputs.forEach(input => {
                    // Gunakan 'keyup' untuk text dan 'change' untuk select
    ...
    $(document).ready(function() {
        // Panggil showStep(0) saat halaman dimuat untuk inisialisasi validasi pertama kali
        showStep(0);
        
        // Init Select2 for asal_sekolah with AJAX
        ...
        function updateKelasDropdown() {
            ...
            // Trigger validasi setelah dropdown kelas berubah
